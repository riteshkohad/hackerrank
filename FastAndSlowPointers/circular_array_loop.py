...
#     slow_direction = "forward"
#     fast_direction = "forward"

#     # slow = (nums[slow] % len(nums)) + slow  
#     # print(slow)
#     for i in range(len(nums) - 1):
#         sval = (nums[slow] % len(nums)) 
#         fval = (nums[fast + 2] % len(nums)) 
...
if __name__ == "__main__":
    ...
    ret_val = circular_array_loop(input_set)
    print(ret_val)

    # 4-1
    # times / len(ar) = ? + current index 
    # 16 / 5  
